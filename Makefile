...
.PHONY: deps build test it envtest bench soak lint image kind-up kind-deps kind-down e2e e2e-job e2e-load e2e-chaos e2e-soak
...
e2e-job: ; bash e2e/job/build.sh $(FLINK_VERSION)

e2e-load:
	kind load docker-image $(IMAGE) --name siesta
	kind load docker-image siesta-e2e-job:$(FLINK_VERSION) --name siesta

# CI builds the controller image and the job jar once per run and hands them to every job,
# with E2E_BUILD empty so nothing is built twice.
E2E_BUILD ?= image e2e-job
