...
func TestDataRoundTripsEveryField(t *testing.T) {
	...
	in := State{Phase: Suspended, Snapshot: map[string]string{"t-0": "1"}, LastActivityAt: t0, SuspendedAt: t0.Add(time.Hour),
		...
		Generation: 7, ResumedAt: t0.Add(2 * time.Hour), Pending: 42, SourceDown: true, SourceDownSince: t0.Add(3 * time.Hour), SuspendChecked: true, ResumeStalledReported: true, SourcesChecked: "job1"}
	...
	if out.Phase != in.Phase || out.Snapshot["t-0"] != "1" || !out.SuspendedAt.Equal(in.SuspendedAt) || out.Restarts != in.Restarts ||
		out.Generation != 7 || out.Pending != 42 || !out.SourceDown || !out.SourceDownSince.Equal(in.SourceDownSince) || !out.SuspendChecked || !out.ResumeStalledReported || out.SourcesChecked != "job1" || !out.ResumedAt.Equal(in.ResumedAt) {
